...
def calc_lpips(target, ref):
    ...
    for i in range(index[0]):
        img_out = target[i].cuda()
        img_gt = ref[i].cuda()

        score = loss_fn(img_gt, img_out)
        lpips_score.append(score.cpu().detach().numpy())
    total = sum(lpips_score)
    count = len(lpips_score)
    lpips_score = total/count
    return lpips_score

def calc_ssim(target, ref):
    ...
    for i in range(index[0]):
        img_out = target[i]
        img_gt = ref[i]
        img_out = 0.299*img_out[0] + 0.587*img_out[1] + 0.114*img_out[2]
        img_gt = 0.299*img_gt[0] + 0.587*img_gt[1] + 0.114*img_gt[2]

        score = ssim(img_gt, img_out, data_range=1)
        ssim_score.append(score)
    total = sum(ssim_score)
    count = len(ssim_score)
    ssim_score = total/count
    return ssim_score
...
def run_epoch(pipeline, phase, epoch, args, iter_cb=None):
    ...
    def run_sub(dl, extra_optimizer):
        ...
        for it, data in enumerate(dl):
            ...
            ssim_value = calc_ssim(out.cpu().detach().numpy(), target.cpu().detach().numpy())
            lpips_value = calc_lpips(out.cpu().detach().numpy(), target.cpu().detach().numpy())

            ad.add('psnr', psnr_value)
            ad.add('ssim', ssim_value)
            ad.add('lpips', lpips_value)
    ...
    avg_lpips = np.mean(ad['lpips'])
    iter_cb.on_epoch(phase, avg_loss, epoch)
    iter_cb.on_epoch(phase, avg_lpips, epoch)
    iter_cb.on_epoch(phase, avg_ssim, epoch)
    iter_cb.on_epoch(phase, avg_psnr, epoch)
...
if __name__ == '__main__':
    ...
    if args.eval:
        loss, psnr, ssim, lpips_score = run_eval(0, pipeline, args, iter_cb)
        print('VAL LOSS', loss)
        print('psnr', psnr)
        print('ssim', ssim)
