...
def intrinsics_from_dft(xml_file):
    f = 725.0087
    cx = 620.5
    cy = 187
...
def extrinsics_from_txt(pose_path):
    extrins_all = np.loadtxt(pose_path)
    x, y = extrins_all.shape
    view_matrices = []
    img_names = []
    for i in range(x):
        #values = np.array([0, 0, 0, 1])
        extrins = extrins_all[i, :]
        #extrins = np.append(extrins, values).reshape(4, 4)
        extrins = np.array(extrins).reshape(4, 4)
        extrins[:, 1:3] *= -1
        view_matrices.append(extrins)
        image_id = '{:06}'.format(i)
        img_names.append(image_id)
    return view_matrices, img_names
...
def load_scene_data(path):
    ...
    if 'intrinsic_matrix' in config:
        ...
        if apath[-3:] == 'xml':
            ...
            intrinsic_matrix, (width, height) = intrinsics_from_dft(apath)
        else:
