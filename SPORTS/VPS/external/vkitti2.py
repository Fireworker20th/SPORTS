...
class VKITTI2:
    CLASSES = ('Terrain', 'sky', 'Tree', 'Vegetation', 'Building', 'Road',
    ...
    def __init__(
        ...
        #assert os.path.exists(os.path.join(data_root, 'labelTrainids')), \
        #    "It seems that '{}' is not the root folder of cityscapes".format(data_root)
        assert os.path.exists(os.path.join(data_root, 'img')), \
            "img cannot be found."
        assert os.path.exists(os.path.join(data_root, 'gtFine', 'instanceTrainids')), \
            "instanceTrainids cannot be found."
        assert os.path.exists(os.path.join(data_root, 'gtFine', 'labelTrainids')), \
            "labelTrainids cannot be found."

        if pipeline is None:
            pipeline = []

        image_main_dir = os.path.join(data_root, 'img', split)
        gt_dir = os.path.join(data_root, 'gtFine')

        locations = os.listdir(image_main_dir)
        samples = []
        for sample in os.listdir(image_main_dir):
                seq_id, img_id= sample.split('_')
                img_id = img_id[:-4]
                samples.append((int(seq_id), int(img_id)))
    ...
    def prepare_test_img(self, idx):
        ...
        filename = os.path.join(self.img_dir, '{:04d}_{:05d}.jpg'.format(*get_idx))
        results = {
            'img_info': {
                'filename': filename
            }
    ...
    def prepare_val_annotation(self, idx):
        ...
        results = {
            'ann_info': {
                'seg_map': os.path.join(self.gt_dir, 'labelTrainids', 'val',
                                        '{:04d}_{:05d}.png'.format(*get_idx)),
                'inst_map': os.path.join(self.gt_dir, 'instanceTrainids', 'val',
                                        '{:04d}_{:05d}.png'.format(*get_idx)),
            }
        }
        results = self.pre_pipeline(results)
    ...
    def prepare_train_img(self, idx):
        ...
        results = {
            ...
            'ann_info': {
                'seg_map': os.path.join(self.gt_dir, 'labelTrainids', 'train',
                                        '{:04d}_{:05d}.png'.format(*get_idx)),
                'inst_map': os.path.join(self.gt_dir, 'instanceTrainids', 'train',
                                        '{:04d}_{:05d}.png'.format(*get_idx)),
            }
        }
        results = self.pre_pipeline(results)
...
if __name__ == '__main__':
    ...
    train_pipelines = [
        ...
        dict(type='Resize', img_scale=(375, 1242), ratio_range=[0.5, 2.0], keep_ratio=True),
        dict(type='RandomFlip', flip_ratio=0.5),
        dict(type='RandomCrop', crop_size=(375, 1242)),
    ...
    data = VKITTI2(
        ...
        data_root='/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/vkitti2/ALL_clone',
        split='train',
        test_mode=False
    )
    for item in data:
