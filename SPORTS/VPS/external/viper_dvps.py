...
class VIPERDVPSDataset:
    CLASSES = ('sky', 'road', 'sidewalk', 'terrain', 'tree', 'vegetation',
               'building', 'infrastructure', 'fence', 'billboard', 'trafficlight',
               'trafficsign', 'mobilebarrier', 'firehydrant', 'chair', 'trash', 
               'trashcan', 'person', 'motorcycle', 'car', 'van', 'bus', 'truck')
    ...
    def __init__(self,
        ...
        video_seq_dir = os.path.join(data_root, 'video', split)
        assert os.path.exists(video_seq_dir)
        assert 'leftImg8bit' not in video_seq_dir

        self.num_thing_classes = 10
        self.num_stuff_classes = 13
    ...
    def evaluate(
        ...
        # logger and metric
        thing_knet2real = [10, 13, 14, 16, 17, 18, 19, 20, 21, 22]
