...
gt_dir = '/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/viper/viper_val/val/'
...
def eval(element):
    ...
    if depth_thres > 0:
        ...
        pred_in_depth_mask[ignored_pred_mask] = 23 * max_ins
...
def main():
    ...
    things_index = np.zeros((23,)).astype(bool)
    things_index[13] = True
    things_index[14] = True
    things_index[15] = True
    things_index[16] = True
    things_index[17] = True
    things_index[18] = True
    things_index[19] = True
    things_index[20] = True
    things_index[21] = True
    things_index[22] = True
    ...
    for i in [1, 2, 3, 4, 5, 6]:
        ...
        epsilon = 1e-10
        iou_per_class = iou_per_class.sum(axis=0)[:23]
        tp_per_class = tp_per_class.sum(axis=0)[:23]
        fn_per_class = fn_per_class.sum(axis=0)[:23]
        fp_per_class = fp_per_class.sum(axis=0)[:23]
    ...
    print("----------------final-----------------")
    iou_per_class_all = np.concatenate(iou_per_class_all, axis=0).sum(axis=0)[:23]
    tp_per_class_all = np.concatenate(tp_per_class_all, axis=0).sum(axis=0)[:23]
    fn_per_class_all = np.concatenate(fn_per_class_all, axis=0).sum(axis=0)[:23]
    fp_per_class_all = np.concatenate(fp_per_class_all, axis=0).sum(axis=0)[:23]
