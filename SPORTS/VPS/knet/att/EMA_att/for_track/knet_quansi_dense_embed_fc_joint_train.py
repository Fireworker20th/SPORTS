...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def __init__(self,
        ...
        if self.fusion:
            ...
            for p in self.fusion_conv2.parameters():
                p.requires_grad = False

            for p in self.fusion_conv3.parameters():
                p.requires_grad = False

            for p in self.fusion_conv4.parameters():
                p.requires_grad = False
            ...
            for p in self.softmax.parameters():
                p.requires_grad = False
            for p in self.agp.parameters():
                p.requires_grad = False
            for p in self.pool_h.parameters():
                p.requires_grad = False
            for p in self.pool_w.parameters():
                p.requires_grad = False
            for p in self.gn.parameters():
                p.requires_grad = False
            for p in self.conv1x1.parameters():
                p.requires_grad = False
            for p in self.conv3x3.parameters():
                p.requires_grad = False

            self.softmax1 = nn.Softmax(-1)
            self.agp1 = nn.AdaptiveAvgPool2d((1, 1))
            self.pool_h1 = nn.AdaptiveAvgPool2d((None, 1))
            self.pool_w1 = nn.AdaptiveAvgPool2d((1, None))
            self.gn1 = nn.GroupNorm(channels // self.groups, channels // self.groups)
            self.conv1x11 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=1, stride=1,
                                      padding=0)
            self.conv3x31 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=3, stride=1,
                                      padding=1)

            for p in self.softmax1.parameters():
                p.requires_grad = False
            for p in self.agp1.parameters():
                p.requires_grad = False
            for p in self.pool_h1.parameters():
                p.requires_grad = False
            for p in self.pool_w1.parameters():
                p.requires_grad = False
            for p in self.gn1.parameters():
                p.requires_grad = False
            for p in self.conv1x11.parameters():
                p.requires_grad = False
            for p in self.conv3x31.parameters():
                p.requires_grad = False

            self.softmax2 = nn.Softmax(-1)
            self.agp2 = nn.AdaptiveAvgPool2d((1, 1))
            self.pool_h2 = nn.AdaptiveAvgPool2d((None, 1))
            self.pool_w2 = nn.AdaptiveAvgPool2d((1, None))
            self.gn2 = nn.GroupNorm(channels // self.groups, channels // self.groups)
            self.conv1x12 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=1, stride=1,
                                      padding=0)
            self.conv3x32 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=3, stride=1,
                                      padding=1)

            for p in self.softmax2.parameters():
                p.requires_grad = False
            for p in self.agp2.parameters():
                p.requires_grad = False
            for p in self.pool_h2.parameters():
                p.requires_grad = False
            for p in self.pool_w2.parameters():
                p.requires_grad = False
            for p in self.gn2.parameters():
                p.requires_grad = False
            for p in self.conv1x12.parameters():
                p.requires_grad = False
            for p in self.conv3x32.parameters():
                p.requires_grad = False

            self.softmax3 = nn.Softmax(-1)
            self.agp3 = nn.AdaptiveAvgPool2d((1, 1))
            self.pool_h3 = nn.AdaptiveAvgPool2d((None, 1))
            self.pool_w3 = nn.AdaptiveAvgPool2d((1, None))
            self.gn3 = nn.GroupNorm(channels // self.groups, channels // self.groups)
            self.conv1x13 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=1, stride=1,
                                      padding=0)
            self.conv3x33 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=3, stride=1,
                                      padding=1)

            for p in self.softmax3.parameters():
                p.requires_grad = False
            for p in self.agp3.parameters():
                p.requires_grad = False
            for p in self.pool_h3.parameters():
                p.requires_grad = False
            for p in self.pool_w3.parameters():
                p.requires_grad = False
            for p in self.gn3.parameters():
                p.requires_grad = False
            for p in self.conv1x13.parameters():
                p.requires_grad = False
            for p in self.conv3x33.parameters():
                p.requires_grad = False
    ...
    def _freeze_detector(self):
        ...
        self.detector = [
            self.backbone, self.neck, self.rpn_head, self.roi_head
        ]
    ...
    def att(self, x, model):
        ...
        if model == 1:
            group_x = x.reshape(b * self.groups, -1, h, w)  # b*g,c//g,h,w
            x_h = self.pool_h1(group_x)
            x_w = self.pool_w1(group_x).permute(0, 1, 3, 2)
            hw = self.conv1x11(torch.cat([x_h, x_w], dim=2))
            x_h, x_w = torch.split(hw, [h, w], dim=2)
            x1 = self.gn1(group_x * x_h.sigmoid() * x_w.permute(0, 1, 3, 2).sigmoid())
            x2 = self.conv3x31(group_x)
            x11 = self.softmax1(self.agp1(x1).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
            x12 = x2.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
            x21 = self.softmax1(self.agp1(x2).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
            x22 = x1.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
            weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22)).reshape(b * self.groups, 1, h, w)
        if model == 2:
            group_x = x.reshape(b * self.groups, -1, h, w)  # b*g,c//g,h,w
            x_h = self.pool_h2(group_x)
            x_w = self.pool_w2(group_x).permute(0, 1, 3, 2)
            hw = self.conv1x12(torch.cat([x_h, x_w], dim=2))
            x_h, x_w = torch.split(hw, [h, w], dim=2)
            x1 = self.gn2(group_x * x_h.sigmoid() * x_w.permute(0, 1, 3, 2).sigmoid())
            x2 = self.conv3x32(group_x)
            x11 = self.softmax2(self.agp2(x1).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
            x12 = x2.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
            x21 = self.softmax2(self.agp2(x2).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
            x22 = x1.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
            weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22)).reshape(b * self.groups, 1, h, w)
        if model == 3:
            group_x = x.reshape(b * self.groups, -1, h, w)  # b*g,c//g,h,w
            x_h = self.pool_h3(group_x)
            x_w = self.pool_w3(group_x).permute(0, 1, 3, 2)
            hw = self.conv1x13(torch.cat([x_h, x_w], dim=2))
            x_h, x_w = torch.split(hw, [h, w], dim=2)
            x1 = self.gn3(group_x * x_h.sigmoid() * x_w.permute(0, 1, 3, 2).sigmoid())
            x2 = self.conv3x33(group_x)
            x11 = self.softmax3(self.agp3(x1).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
            x12 = x2.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
            x21 = self.softmax3(self.agp3(x2).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
            x22 = x1.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
            weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22)).reshape(b * self.groups, 1, h, w)
    ...
    def load_img(self, img_file):
        img = mmcv.imread(img_file, channel_order='rgb', flag=self.color_type)
        img = mmcv.imnormalize(img, self.mean, self.std, self.to_rgb)
        pad_img = mmcv.impad_to_multiple(
            img, 32, pad_val=0)
        pad_img = np.expand_dims(pad_img, -1)
        if len(pad_img.shape) == 3:
            pad_img = np.ascontiguousarray(pad_img.transpose(2, 0, 1))
        else:
            pad_img = np.ascontiguousarray(pad_img.transpose(3, 2, 0, 1))
        pad_img = to_tensor(pad_img)

        return pad_img

    def data_for_fusion(self, img_meta):
        what = img_meta[0]['for_what']
        img_file_cur = img_meta[0]['ori_filename']
        part_cur = img_file_cur.rsplit("_")[1]
        fid_cur = int(part_cur)
        part_ref = "{:06d}".format(fid_cur - 1)
        img_file = img_file_cur.rsplit("_")[0] + "_" + part_ref + "_" + img_file_cur.rsplit("_")[2]
    ...
    def forward_train(self,
        ...
        if self.fusion:
            for i in range(2):
                if i == 0:
                    a = ref_img_metas_new[0]['ori_filename']
                    part = a.rsplit("_")[1]
                    fid = int(part)
                    is_first = (fid == 0)
                    if is_first:
                        x_ref = self.extract_feat(ref_img)
                        ref_rpn_results = self.rpn_head.forward_train(x_ref, x_ref, ref_img_metas_new,
                                                                      ref_gt_masks,
                                                                      ref_labels_gt, ref_gt_sem_seg,
                                                                      ref_gt_sem_cls)
                        (ref_rpn_losses, ref_proposal_feats, ref_x_feats, ref_mask_preds,
                         ref_cls_scores) = ref_rpn_results
                        ref_x_feats2 = ref_x_feats
                        ref_x_feats3 = ref_x_feats
                        ref_x_feats4 = ref_x_feats
                    else:
                        flow, depth, pose, img_ref = self.data_for_fusion(ref_img_metas_new)
                        x = self.extract_feat(ref_img)
                        x_ref = self.extract_feat(img_ref)
                        for model in range(4):
                            if model == 0:
                                x_fusion_map1 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                                ref_rpn_results = self.rpn_head.forward_train(x_fusion_map1, x_fusion_map1,
                                                                              ref_img_metas_new, ref_gt_masks,
                                                                              ref_labels_gt, ref_gt_sem_seg,
                                                                              ref_gt_sem_cls)
                                (ref_rpn_losses, ref_proposal_feats, ref_x_feats, ref_mask_preds,
                                 ref_cls_scores) = ref_rpn_results
                            elif model == 1:
                                x_fusion_map2 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                                ref_rpn_results2 = self.rpn_head.forward_train(x_fusion_map2, x_fusion_map2,
                                                                               ref_img_metas_new, ref_gt_masks,
                                                                               ref_labels_gt, ref_gt_sem_seg,
                                                                               ref_gt_sem_cls)
                                (ref_rpn_losses2, ref_proposal_feats2, ref_x_feats2, ref_mask_preds2,
                                 ref_cls_scores2) = ref_rpn_results2
                            elif model == 2:
                                x_fusion_map3 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                                ref_rpn_results3 = self.rpn_head.forward_train(x_fusion_map3, x_fusion_map3,
                                                                               ref_img_metas_new, ref_gt_masks,
                                                                               ref_labels_gt, ref_gt_sem_seg,
                                                                               ref_gt_sem_cls)
                                (ref_rpn_losses3, ref_proposal_feats3, ref_x_feats3, ref_mask_preds3,
                                 ref_cls_scores3) = ref_rpn_results3
                            elif model == 3:
                                x_fusion_map4 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                                ref_rpn_results4 = self.rpn_head.forward_train(x_fusion_map4, x_fusion_map4,
                                                                               ref_img_metas_new, ref_gt_masks,
                                                                               ref_labels_gt, ref_gt_sem_seg,
                                                                               ref_gt_sem_cls)
                                (ref_rpn_losses4, ref_proposal_feats4, ref_x_feats4, ref_mask_preds4,
                                 ref_cls_scores4) = ref_rpn_results4

                elif i == 1:
                    a = img_metas[0]['ori_filename']
                    part = a.rsplit("_")[1]
                    fid = int(part)
                    is_first = (fid == 0)
                    if is_first:
                        x = self.extract_feat(img)
                        rpn_results = self.rpn_head.forward_train(x, x, img_metas, gt_masks,
                        ...
                        flow, depth, pose, img_ref = self.data_for_fusion(img_metas)
                        x = self.extract_feat(img)
                        x_ref = self.extract_feat(img_ref)
                        for model1 in range(4):
                            if model1 == 0:
                                x_fusion_map1 = self.for_fusion_map(x_ref, x, flow, depth, pose, model1)
                                rpn_results = self.rpn_head.forward_train(x_fusion_map1, x_fusion_map1, img_metas,
                                                                          gt_masks,
                                                                          gt_labels, gt_sem_seg,
                                                                          gt_sem_cls)
                                (rpn_losses, proposal_feats, x_feats, mask_preds,
                                ...
                                x_fusion_map2 = self.for_fusion_map(x_ref, x, flow, depth, pose, model1)
                                rpn_results2 = self.rpn_head.forward_train(x_fusion_map2, x_fusion_map2, img_metas,
                                                                           gt_masks,
                                                                           gt_labels, gt_sem_seg,
                                                                           gt_sem_cls)
                                ...
                                x_fusion_map3 = self.for_fusion_map(x_ref, x, flow, depth, pose, model1)
                                rpn_results3 = self.rpn_head.forward_train(x_fusion_map3, x_fusion_map3, img_metas,
                                                                           gt_masks,
                                                                           gt_labels, gt_sem_seg,
                                                                           gt_sem_cls)
                                ...
                                x_fusion_map4 = self.for_fusion_map(x_ref, x, flow, depth, pose, model1)
                                rpn_results4 = self.rpn_head.forward_train(x_fusion_map4, x_fusion_map4, img_metas,
                                                                           gt_masks,
                                                                           gt_labels, gt_sem_seg,
                                                                           gt_sem_cls)
                                (rpn_losses4, proposal_feats4, x_feats4, mask_preds4,
                                 cls_scores4) = rpn_results4
    ...
    def simple_test(self, img, img_metas, rescale=False, ref_img=None, **kwargs):
        ...
        if self.fusion:
            ...
            if is_first:
                ...
                for model in range(4):
                    if model == 0:
                        ...
                        x_fusion_map1 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                        rpn_results = self.rpn_head.simple_test_rpn(x_fusion_map1, x_fusion_map1, img_metas)
                        (proposal_feats, x_feats, mask_preds, cls_scores,
                         seg_preds) = rpn_results
                        ...
                        x_ref = self.x_ref_memory
                        x_fusion_map2 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                        rpn_results1 = self.rpn_head.simple_test_rpn(x_fusion_map2, x_fusion_map2, img_metas)
                        ...
                        x_ref = self.x_ref_memory
                        x_fusion_map3 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                        rpn_results2 = self.rpn_head.simple_test_rpn(x_fusion_map3, x_fusion_map3, img_metas)
                        ...
                        x_ref = self.x_ref_memory
                        x_fusion_map4 = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                        rpn_results3 = self.rpn_head.simple_test_rpn(x_fusion_map4, x_fusion_map4, img_metas)
            ...
            if self.link_previous:
                ...
                self.x_feats_memory = x_feats3
                self.mask_preds_memory = scaled_mask_preds
                self.image_metas_memory = img_metas
                self.x_fusion_memory = x_fusion_map1
                self.x_fusion_memory1 = x_fusion_map2
                self.x_fusion_memory2 = x_fusion_map3
                self.x_fusion_memory3 = x_fusion_map4
                self.x_ref_memory = x
            else:
