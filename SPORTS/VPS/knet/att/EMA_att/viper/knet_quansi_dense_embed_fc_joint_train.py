...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def att(self, x, model):
        ...
        def process_model(model, x):
            ...
            conv1x1 = getattr(self, f'conv1x1{model}')
            conv3x3 = getattr(self, f'conv3x3{model}')
            gn = getattr(self, f'gn{model}')
            ...
            # 处理卷积
            hw = conv1x1(torch.cat([x_h, x_w], dim=2))
            x_h, x_w = torch.split(hw, [h, w], dim=2)
            x1 = gn(group_x * x_h.sigmoid() * x_w.permute(0, 1, 3, 2).sigmoid())
            ...
            del x_h, x_w, hw, x1, x2, x11, x12, x21, x22
            return group_x * weights.sigmoid()

        # 处理不同模型
        if model in [0, 1, 2, 3]:
            return process_model(model, x).reshape(b, c, h, w)
        else:
            raise ValueError(f"Unsupported model index: {model}")

    def fusion_module(self, features, model):
        # 选择对应的融合卷积层
        fusion_convs = [self.fusion_conv1, self.fusion_conv2, self.fusion_conv3, self.fusion_conv4]

        # 确保模型索引在有效范围内
        if model < 0 or model >= len(fusion_convs):
            raise ValueError(f"Invalid model index: {model}")

        # 处理每个特征图
        for k, v in features.items():
            v = self.att(v, model)
            # 应用对应的卷积层
            x = fusion_convs[model](v)
            features[k] = x
    ...
    def flow_transport_feature_with_depth(self, features, ori_flow, ori_depth):  # 3xHxW
        ...
        for k, feat in features.items():
            ...
            del ref_depth, ref_flow, feat, query_mask, mask
    ...
    def data_for_fusion(self, ref_img_metas_new):
        ...
        del part1, part2, part3, name, depth_name, pose_name, pose_cur_name, depth_file, pose_file, pose_cur_file
    ...
    def forward_train(self,
        ...
        if self.fusion:
            ...
            flow, depth, pose = self.data_for_fusion(ref_img_metas_new)

            for model in range(4):
                if model == 0:
                    x_fusion_map = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                    rpn_results = self.rpn_head.forward_train(x_fusion_map, x_fusion_map, img_metas,
                                                              gt_masks,
                    ...
                    x_fusion_map = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                    rpn_results1 = self.rpn_head.simple_test_rpn(x_fusion_map, x_fusion_map, img_metas)
                    (_,x_feats2,_,_,_) = rpn_results1
                elif model == 2:
                    x_fusion_map = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                    rpn_results1 = self.rpn_head.simple_test_rpn(x_fusion_map, x_fusion_map, img_metas)
                    (_,x_feats3,_,_,_) = rpn_results1
                elif model == 3:
                    x_fusion_map = self.for_fusion_map(x_ref, x, flow, depth, pose, model)
                    rpn_results1 = self.rpn_head.simple_test_rpn(x_fusion_map, x_fusion_map, img_metas)
                    (_,x_feats4,_,_,_) = rpn_results1

            del x_fusion_map, x, pose, flow, depth
            ...
            del x_feats, x_feats2, x_feats3, x_feats4
