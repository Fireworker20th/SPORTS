...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def __init__(self,
        ...
        if self.fusion:
            ...
            self.fusion_conv1 = nn.Conv2d(512, 256, kernel_size=7, stride=1, padding=3)

            self.fusion_conv2 = nn.Conv2d(512, 256, kernel_size=7, stride=1, padding=3)

            self.fusion_conv3 = nn.Conv2d(512, 256, kernel_size=7, stride=1, padding=3)

            self.fusion_conv4 = nn.Conv2d(512, 256, kernel_size=7, stride=1, padding=3)
            ...
            self.softmax0 = nn.Softmax(-1)
            self.agp0 = nn.AdaptiveAvgPool2d((1, 1))
            self.pool_h0 = nn.AdaptiveAvgPool2d((None, 1))
            self.pool_w0 = nn.AdaptiveAvgPool2d((1, None))
            self.gn0 = nn.GroupNorm(channels // self.groups, channels // self.groups)
            self.conv1x10 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=1, stride=1,
                                      padding=0)
            self.conv3x30 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=3, stride=1,
                                      padding=1)
            ...
            if gt_semantic_seg is not None:
                ...
                if self.cityscapes or self.vipseg:
                    ...
                    sem_labels, sem_seg = sem2ins_masks_kitti_step(
                        ...
                        ignore_label=self.ignore_label,
                        label_shift=10,
                        thing_label_in_seg=(10, 13, 14, 16, 17, 18, 19, 20, 21, 22))
                else:
    ...
    def data_for_fusion(self, img_meta, ref_img_metas_new):
        ...
        depth_name = part1 + '_' + part2 + '.png'
        pose_name = part1 + '_' + part2 + '.txt'
        pose_cur_name = part1 + '_' + part3 + '.txt'
        depth_file = os.path.join(
            "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/viper/video/depth",
            depth_name)
    ...
    def get_semantic_seg(self, panoptic_seg, segments_info):
        kitti_step2cityscpaes = [10, 13, 14, 16, 17, 18, 19, 20, 21, 22]
