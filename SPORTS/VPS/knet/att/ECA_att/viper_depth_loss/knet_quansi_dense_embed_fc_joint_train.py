...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def load_depth(self, depth_file):
        depth_pred = np.load(depth_file)
        depth_pred = cv2.resize(depth_pred, (1080, 1920))
        depth_pred = np.transpose(depth_pred, (1, 0))
        depth = np.array(depth_pred, dtype=np.float64)
        # depth[depth <= 0.00763] = 0.00763
        depth = np.array(1.0 / depth) * 500
        o_h, o_w = 1080, 1920
        t_h, t_w = 1088, 1920
        pad_img = np.zeros((t_h, t_w), dtype=np.float64)
        pad_img[:o_h, :o_w] = depth
        depth = torch.as_tensor(pad_img)
        return depth
    ...
    def load_flow(self, flow_file):
        ...
        flow_pred = cv2.resize(flow_pred, (1080, 1920))
        flow_pred = np.transpose(flow_pred, (1, 0, 2))

        o_h, o_w = 1080, 1920
        t_h, t_w = 1088, 1920
        pad_img = np.zeros((t_h, t_w, 2), dtype=np.float64)
        pad_img[:o_h, :o_w, :] = flow_pred
    ...
    def wrap_in_stage1(self, x, images, flow, depth, pose, model):

        if self.flow_transport:
            query_img = self.flow_transport_img(images, flow)  # 1x3xHxW
        elif self.flow_depth_transport:
            if self.depth_proj_op:
                depth = self.pose_transport_depth(depth, pose)
            query_img = self.flow_transport_img_with_depth(images, flow, depth)
        ...
        x_ref = self.extract_feat(images)
        ...
        for k, feat in x_fusion.items():
            x_fusion[k] = torch.cat((feat[1], self.alpha * feat[0]), dim=0).unsqueeze(0)
        # p2 1x512x96x312, p3 1x512x48x156, p4, p5
        features = self.fusion_module(x_fusion, model)
    ...
    def flow_transport_img_with_depth(self, image, ori_flow, ori_depth):  # 3xHxW
        ...
        ref_depth = self.resize(ori_depth.unsqueeze(0).unsqueeze(0), image.shape[-2:]).squeeze(0).squeeze(
            0).cpu().numpy()
    ...
    def pose_transport_depth(self, ori_depth, pose):
        ...
        depth = torch.as_tensor(Zc0.reshape(rows, cols)).to(self.device)
        return depth
    ...
    def data_for_fusion(self, img_meta, ref_img_metas_new):
        ...
        part1 = int(img_file.rsplit("_")[0])
        part1 = "{:04d}".format(part1)
        part2 = img_file.rsplit("_")[1]
        part2 = part2.split(".")
        part2 = int(part2[0])

        part3 = "{:05d}".format(part2 + 1)
        part2 = "{:05d}".format(part2)
        name = part1 + '_' + part2 + '.npy'
        pose_name = part1 + '_' + part2 + '.txt'
        pose_cur_name = part1 + '_' + part3 + '.txt'
        depth_file = os.path.join(
            "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/viper/video/aug_depth",
            name)
        flow_file = os.path.join(
            "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/viper/video/flow",
            name)
        pose_file = os.path.join(
            "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/viper/video/pose",
            pose_name)
        pose_cur_file = os.path.join(
            "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/viper/video/pose",
            pose_cur_name)
    ...
    def for_fusion_map(self, x_feats_fusion):
    ...
    def forward_train(self,
        ...
        if self.fusion:
            ...
            for model in range(4):
                if model == 0:
                    ...
                    x_feats_fusion_loss1 = self.wrap_in_stage1(x, ref_img, flow, depth, pose, model)

                    x_fusion_map1 = self.for_fusion_map(x_feats_fusion1)
                    x_lossmap1 = self.for_fusion_map(x_feats_fusion_loss1)

                    l_fea1 = (self.l1loss(x_feats_fusion1['p2'], x_feats_fusion_loss1['p2']) +
                              self.l1loss(x_feats_fusion1['p3'], x_feats_fusion_loss1['p3']) +
                              self.l1loss(x_feats_fusion1['p4'], x_feats_fusion_loss1['p4']) +
                              self.l1loss(x_feats_fusion1['p5'], x_feats_fusion_loss1['p5']))

                    rpn_results = self.rpn_head.forward_train(x_fusion_map1, x_fusion_map1, img_metas, gt_masks,
                                                              gt_labels, gt_sem_seg,
                                                              gt_sem_cls)
                    (rpn_losses, proposal_feats, x_feats, mask_preds,
                     cls_scores) = rpn_results
                    rpn_results_loss = self.rpn_head.forward_train(x_lossmap1, x_lossmap1, img_metas, gt_masks,
                                                              gt_labels, gt_sem_seg,
                                                              gt_sem_cls)
                    (rpn_losses_loss, proposal_feats_loss, x_feats_loss, mask_preds_loss,
                     cls_scores_loss) = rpn_results_loss

                    L_rec1 = self.l1loss(mask_preds, mask_preds_loss) + \
                             self.l1loss(proposal_feats, proposal_feats)

                elif model == 1:
                    x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                    x_feats_fusion2 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                    x_feats_fusion_loss2 = self.wrap_in_stage1(x, ref_img, flow, depth, pose, model)

                    x_fusion_map2 = self.for_fusion_map(x_feats_fusion2)
                    x_lossmap2 = self.for_fusion_map(x_feats_fusion_loss2)

                    l_fea2 = (self.l1loss(x_feats_fusion2['p2'], x_feats_fusion_loss2['p2']) +
                              self.l1loss(x_feats_fusion2['p3'], x_feats_fusion_loss2['p3']) +
                              self.l1loss(x_feats_fusion2['p4'], x_feats_fusion_loss2['p4']) +
                              self.l1loss(x_feats_fusion2['p5'], x_feats_fusion_loss2['p5']))

                    rpn_results1 = self.rpn_head.forward_train(x_fusion_map2, x_fusion_map2, img_metas, gt_masks,
                                                              gt_labels, gt_sem_seg,
                                                              gt_sem_cls)
                    (rpn_losses1, proposal_feats1, x_feats1, mask_preds1,
                     cls_scores1) = rpn_results1
                    rpn_results_loss1 = self.rpn_head.forward_train(x_lossmap2, x_lossmap2, img_metas, gt_masks,
                                                                   gt_labels, gt_sem_seg,
                                                                   gt_sem_cls)
                    (rpn_losses_loss1, proposal_feats_loss1, x_feats_loss1, mask_preds_loss1,
                     cls_scores_loss1) = rpn_results_loss1

                elif model == 2:
                    x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                    x_feats_fusion3 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                    x_feats_fusion_loss3 = self.wrap_in_stage1(x, ref_img, flow, depth, pose, model)

                    x_fusion_map3 = self.for_fusion_map(x_feats_fusion3)
                    x_lossmap3 = self.for_fusion_map(x_feats_fusion_loss3)

                    l_fea3 = (self.l1loss(x_feats_fusion3['p2'], x_feats_fusion_loss3['p2']) +
                              self.l1loss(x_feats_fusion3['p3'], x_feats_fusion_loss3['p3']) +
                              self.l1loss(x_feats_fusion3['p4'], x_feats_fusion_loss3['p4']) +
                              self.l1loss(x_feats_fusion3['p5'], x_feats_fusion_loss3['p5']))

                    rpn_results2 = self.rpn_head.forward_train(x_fusion_map3, x_fusion_map3, img_metas, gt_masks,
                                                               gt_labels, gt_sem_seg,
                                                               gt_sem_cls)
                    (rpn_losses2, proposal_feats2, x_feats2, mask_preds2,
                     cls_scores2) = rpn_results2
                    rpn_results_loss2 = self.rpn_head.forward_train(x_lossmap3, x_lossmap3, img_metas, gt_masks,
                                                                    gt_labels, gt_sem_seg,
                                                                    gt_sem_cls)
                    (rpn_losses_loss2, proposal_feats_loss2, x_feats_loss2, mask_preds_loss2,
                     cls_scores_loss2) = rpn_results_loss2

                elif model == 3:
                    x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                    x_feats_fusion4 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                    x_feats_fusion_loss4 = self.wrap_in_stage1(x, ref_img, flow, depth, pose, model)

                    x_fusion_map4 = self.for_fusion_map(x_feats_fusion4)
                    x_lossmap4 = self.for_fusion_map(x_feats_fusion_loss4)

                    l_fea4 = (self.l1loss(x_feats_fusion4['p2'], x_feats_fusion_loss4['p2']) +
                              self.l1loss(x_feats_fusion4['p3'], x_feats_fusion_loss4['p3']) +
                              self.l1loss(x_feats_fusion4['p4'], x_feats_fusion_loss4['p4']) +
                              self.l1loss(x_feats_fusion4['p5'], x_feats_fusion_loss4['p5']))

                    rpn_results3 = self.rpn_head.forward_train(x_fusion_map4, x_fusion_map4, img_metas, gt_masks,
                                                               gt_labels, gt_sem_seg,
                                                               gt_sem_cls)
                    (rpn_losses3, proposal_feats3, x_feats3, mask_preds3,
                     cls_scores3) = rpn_results3
                    rpn_results_loss3 = self.rpn_head.forward_train(x_lossmap4, x_lossmap4, img_metas, gt_masks,
                                                                    gt_labels, gt_sem_seg,
                                                                    gt_sem_cls)
                    (rpn_losses_loss3, proposal_feats_loss3, x_feats_loss3, mask_preds_loss3,
                     cls_scores_loss3) = rpn_results_loss3
            ...
            if self.link_previous:
                ...
                losses_loss, object_feats_loss, cls_scores_loss, mask_preds_loss, scaled_mask_preds_loss, object_feats_track_loss = self.roi_head.forward_train_with_previous(
                    x_feats_loss1,
                    x_feats_loss2,
                    x_feats_loss3,
                    proposal_feats,
                ...
                L_rec2 = self.l1loss(mask_preds, mask_preds_loss) + \
                         self.l1loss(cls_scores, cls_scores_loss) + \
                         self.l1loss(object_feats_track, object_feats_track_loss)
        ...
        losses['loss_f1'] = l_fea1
        losses['loss_f2'] = l_fea2
        losses['loss_f3'] = l_fea3
        losses['loss_f4'] = l_fea4
        losses['loss_rec1'] = L_rec1
        losses['loss_rec2'] = L_rec2
                ...
                for model in range(4):
                    if model == 0:
                        ...
                        x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                    'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                        x_feats_fusion1 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                        x_fusion_map = self.for_fusion_map(x_feats_fusion1)
                        rpn_results = self.rpn_head.simple_test_rpn(x_fusion_map, x_fusion_map, img_metas)
                        (proposal_feats, x_feats, mask_preds, cls_scores,
                         seg_preds) = rpn_results
                        ...
                        x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                    'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                        x_feats_fusion2 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                        x_fusion_map1 = self.for_fusion_map(x_feats_fusion2)
                        rpn_results1 = self.rpn_head.simple_test_rpn(x_fusion_map1, x_fusion_map1, img_metas)
                        ...
                        x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                    'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                        x_feats_fusion3 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                        x_fusion_map2 = self.for_fusion_map(x_feats_fusion3)
                        rpn_results2 = self.rpn_head.simple_test_rpn(x_fusion_map2, x_fusion_map2, img_metas)
                        ...
                        x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                                    'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                        x_feats_fusion4 = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
                        x_fusion_map3 = self.for_fusion_map(x_feats_fusion4)
                        rpn_results3 = self.rpn_head.simple_test_rpn(x_fusion_map3, x_fusion_map3, img_metas)
