...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def __init__(self,
        ...
        if self.fusion:
            ...
            self.avg_pool1 = nn.AdaptiveAvgPool2d(1)  # squeeze
            ...
            self.avg_pool2 = nn.AdaptiveAvgPool2d(1)  # squeeze
            ...
            self.avg_pool3 = nn.AdaptiveAvgPool2d(1)  # squeeze
    ...
    def att(self, x, model):
        b, c, _, _ = x.size()  # channel attention
        if model == 0:
            out = self.avg_pool(x).view(b, c)
            out = self.fc(out).view(b, c, 1, 1)
        if model == 1:
            out = self.avg_pool1(x).view(b, c)
            out = self.fc1(out).view(b, c, 1, 1)
        if model == 2:
            out = self.avg_pool2(x).view(b, c)
            out = self.fc2(out).view(b, c, 1, 1)
        if model == 3:
            out = self.avg_pool3(x).view(b, c)
            out = self.fc3(out).view(b, c, 1, 1)
        return out * x
