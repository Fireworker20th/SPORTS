...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def for_fusion_map(self, img_ref, x_ref, x, flow, depth, pose, model):
        x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                    'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
        x_feats_fusion = self.wrap_in_stage2(x_fusion, flow, depth, pose, model)
        x_feats_fusion_loss = self.wrap_in_stage1(x, img_ref, flow, depth, pose, model)

        l_fea = (self.l1loss(x_feats_fusion['p2'], x_feats_fusion_loss['p2']) +
                 self.l1loss(x_feats_fusion['p3'], x_feats_fusion_loss['p3']) +
                 self.l1loss(x_feats_fusion['p4'], x_feats_fusion_loss['p4']) +
                 self.l1loss(x_feats_fusion['p5'], x_feats_fusion_loss['p5']))
    ...
    def forward_train(self,
            ...
            for i in range(2):
                if i == 0:
                    ...
                    if is_first:
                        ...
                        ref_loss_f1 = self.l1loss(ref_x_feats, ref_x_feats)
                        ref_loss_f2 = ref_loss_f1
                        ref_loss_f3 = ref_loss_f1
                        ref_loss_f4 = ref_loss_f1
                    else:
                        ...
                        for model in range(4):
                            if model == 0:
                                x_fusion_map1, ref_loss_f1 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model)
                                ...
                                x_fusion_map2, ref_loss_f2 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model)
                                ...
                                x_fusion_map3, ref_loss_f3 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model)
                                ...
                                x_fusion_map4, ref_loss_f4 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model)
                    ...
                    if is_first:
                        ...
                        x_feats3 = x_feats
                        x_feats4 = x_feats
                        loss_f1 = self.l1loss(x_feats, x_feats)
                        loss_f2 = loss_f1
                        loss_f3 = loss_f1
                        loss_f4 = loss_f1
                    else:
                        ...
                        for model1 in range(4):
                            if model1 == 0:
                                x_fusion_map1, loss_f1 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model1)
                                ...
                                x_fusion_map2, loss_f2 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model1)
                                ...
                                x_fusion_map3, loss_f3 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model1)
                                ...
                                x_fusion_map4, loss_f4 = self.for_fusion_map(img_ref, x_ref, x, flow, depth, pose, model1)
        ...
        losses['ref_loss_f1'] = ref_loss_f1
        losses['ref_loss_f2'] = ref_loss_f2
        losses['ref_loss_f3'] = ref_loss_f3
        losses['ref_loss_f4'] = ref_loss_f4
        losses['loss_f1'] = loss_f1
        losses['loss_f2'] = loss_f2
        losses['loss_f3'] = loss_f3
        losses['loss_f4'] = loss_f4
