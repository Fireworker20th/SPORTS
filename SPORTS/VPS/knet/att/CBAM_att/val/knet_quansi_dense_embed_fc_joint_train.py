...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def att(self, x, model):
        ...
        if model == 1:
            avg_out = self.fc1(self.avg_pool1(x))
            max_out = self.fc1(self.max_pool1(x))
            out = avg_out + max_out
            out = self.sigmoid1(out)
        if model == 2:
            avg_out = self.fc2(self.avg_pool2(x))
            max_out = self.fc2(self.max_pool2(x))
            out = avg_out + max_out
            out = self.sigmoid2(out)
        if model == 3:
            avg_out = self.fc3(self.avg_pool3(x))
            max_out = self.fc3(self.max_pool3(x))
            out = avg_out + max_out
