...
class ConvKernelHead(nn.Module):
    ...
    def _decode_init_proposals(self, img, img_fs, img_metas):
        num_imgs = len(img_metas)

        localization_feats = self.localization_fpn(img)
        localization_feats_fs = self.localization_fpn(img_fs)

        ## thing branch
        if isinstance(localization_feats_fs, list):
            loc_feats = localization_feats_fs[0]
        else:
            loc_feats = localization_feats_fs
    ...
    def forward_train(self,
        ...
        num_imgs = len(img_metas)
        results = self._decode_init_proposals(img, img_fs, img_metas)
    ...
    def simple_test_rpn(self, img, img_fs, img_metas):
        """Forward function in testing stage."""
        return self._decode_init_proposals(img, img_fs, img_metas)
