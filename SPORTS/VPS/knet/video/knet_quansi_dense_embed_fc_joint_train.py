...
class VideoKNetQuansiEmbedFCJointTrain(BaseDetector):
    ...
    def load_depth(self, depth_file):
        ...
        depth_pred = cv2.resize(depth_pred, (375, 1242))
        depth_pred = np.transpose(depth_pred, (1, 0))
        depth = np.array(depth_pred)
        depth[depth <= 0.00763] = 0.00763
        depth = np.array(1.0 / depth)
        o_h, o_w = 375, 1242
        t_h, t_w = 384, 1248
        pad_img = np.zeros((t_h, t_w), dtype=np.int32)
    ...
    def load_flow(self, flow_file):
        ...
        # flow_pred = cv2.resize(flow_pred, (375, 1242))
    ...
    def wrap_in_stage2(self, features,flow,model):
    ...
    def data_for_fusion(self, img_meta, ref_img_metas_new):
        ...
        #     "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/vkitti2/ALL_clone/video/depth",
        #     name)
        # flow_file = os.path.join(
        #     "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/vkitti2/ALL_clone/video/full_flow",
        #     name)
        # pose_file = os.path.join(
        #     "/media/jydai/C0FED904FED8F39E/download_jyd/project/Video-K-Net-main/data/vkitti2/ALL_clone/video/pose_for_val",
        #     pose_name)
    ...
    def simple_test(self, img, img_metas, rescale=False, ref_img=None, **kwargs):
        ...
        if self.fusion:
            ...
            if is_first:
                ...
                flow = self.data_for_fusion(img_metas, self.image_metas_memory)
                x = self.extract_feat(img)
                ...
                x_feats_fusion = self.wrap_in_stage2(x_fusion,flow,0)

                x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                            'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                x_feats_fusion2 = self.wrap_in_stage2(x_fusion, flow, 1)

                x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                            'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                x_feats_fusion3 = self.wrap_in_stage2(x_fusion, flow, 2)

                x_fusion = {'p2': torch.cat((x_ref[0], x[0]), dim=0), 'p3': torch.cat((x_ref[1], x[1]), dim=0),
                            'p4': torch.cat((x_ref[2], x[2]), dim=0), 'p5': torch.cat((x_ref[3], x[3]), dim=0)}
                x_feats_fusion4 = self.wrap_in_stage2(x_fusion, flow, 3)
                fusion_map = [0, 0, 0, 0]
            ...
            if self.link_previous:
                ...
                self.x_fusion_memory1 = x_fusion_map
                self.x_fusion_memory2 = x_fusion_map
                self.x_fusion_memory3 = x_fusion_map
                self.x_ref_memory = x
            else:
