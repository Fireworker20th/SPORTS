...
def init():
    ...
    parser.add_argument("--full_flow_dir", default="datasets/viper_pvo/PVO_out/full_flow")
    parser.add_argument("--depth_dir", default="datasets/viper_pvo/PVO_out/depth")
...
if __name__ == '__main__':
    ...
    db = dataset_factory(['viper'], datapath='datasets/viper_pvo', do_aug=False,
                         n_frames=n_frames, flow_label=False, aug_graph=False,
                         scene_id=scene_id, need_inv=False, build_mask=False, crop_size=img_size, mode='semisup',
                         rebuild=True,
                         )
    ...
    state_dict = OrderedDict(
        [(k.replace("module.", ""), v) for (k, v) in torch.load(args.weights_file, device).items()])
