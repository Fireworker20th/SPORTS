...
def init():
    ...
    parser.add_argument("--full_flow_dir", default="datasets/kitti/PVO_out/full_flow")
    parser.add_argument("--depth_dir", default="datasets/kitti/PVO_out/depth")
...
if __name__ == '__main__':
    ...
    db = dataset_factory(['kitti'], datapath='datasets/kitti', do_aug=False,
                     n_frames=n_frames, flow_label=False, aug_graph=False,
                         scene_id = scene_id, need_inv=False, build_mask=False, crop_size=img_size, mode='semisup', rebuild=True,
                     )
    train_loader = DataLoader(db, batch_size=1, num_workers=0)
    ...
    for i_batch, item in enumerate(train_loader):
        images, poses, disps, intrinsics = [ x.to('cuda') for x in item if type(x) != list]
        file1, file2 = item[0]
        ...
        img_id = dic[scene_id] + '_' + file1[0].rsplit('.')[0]
        print(img_id)

        full_flow_1 = full_flow.detach().cpu().numpy()

        if save_npy:
            full_flow_1 = cv2.resize(full_flow_1, (370, 1226))
