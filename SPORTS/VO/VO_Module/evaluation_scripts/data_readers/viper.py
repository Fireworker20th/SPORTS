...
class VIPER(RGBDDataset):
    ...
    def __init__(self, scene_id='01', **kwargs):
        self.n_frames = 2
        self.test_split = [x for x in VIPER.scenes if x != scene_id]
        super(VIPER, self).__init__(name='VIPER', **kwargs)
    ...
    def _build_dataset(self):
        ...
        for scene in tqdm(sorted(scenes)):
            ...
            t = poses[:, :3, 3] / VIPER.DEPTH_SCALE
            poses = np.concatenate((t, r), axis=1)

            intrinsics = [VIPER.calib_read()] * len(images)
    ...
    @staticmethod
    def calib_read():
        return np.array([1.2063, 2.1445, 960, 540])
    ...
    def depth_read(depth_file):
        depth = cv2.imread(depth_file, cv2.IMREAD_ANYCOLOR |
                           cv2.IMREAD_ANYDEPTH) / (VIPER.DEPTH_SCALE * 100)
