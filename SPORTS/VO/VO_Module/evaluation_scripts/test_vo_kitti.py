...
def image_stream(datapath, image_size=[240, 808], mode='train',
                 args=None):  # -------------------------------------------
    """ image generator """
    fx, fy, cx, cy = 707.09, 707.09, 601.89, 183.11

    images = np.array(sorted(glob.glob(os.path.join(datapath, 'image_2/*.png'))))
    segments_list = sorted(glob.glob(os.path.join(datapath, 's10_kv2/*.png')))
    ...
    for t, imfile in enumerate(images_list):
        ...
        yield t, image, intrinsics, segm, image_id
...
def init():
    ...
    parser.add_argument("--buffer", type=int, default=1536)
...
if __name__ == '__main__':
    ...
    for (t, image, intrinsics, segm, image_id) in tqdm(image_stream(args.datapath, mode='val', args=args)):
    ...
    gt_file = os.path.join(args.datapath, '10.txt')
    ...
    path = os.path.join(path, 'kitti10')
    ...
    root = os.path.join(root, 'kitti10')
