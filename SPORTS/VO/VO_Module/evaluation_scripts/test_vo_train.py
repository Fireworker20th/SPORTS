...
def image_stream(datapath, image_size=[240, 808], mode='train',
                 args=None):  # -------------------------------------------
    """ image generator """
    ...
    segments_list = sorted(glob.glob(os.path.join(datapath, split[mode], 'panFPN_segm_kv2/*.png')))
    segments_id_list = sorted(glob.glob(os.path.join(datapath, split[mode], 'gt_veiw/*.png')))
    ...
    for t, imfile in enumerate(images_list):
        ...
        id_image = cv2.imread(segments_id_list[t])
        id_image = cv2.resize(id_image, (w1, h1))
        id_image = id_image[:h1 - h1 % 8, :w1 - w1 % 8]
        id_image = torch.as_tensor(id_image).int().permute(2, 0, 1)
        ...
        yield t, image, intrinsics, segm, id_image
...
def init():
    ...
    parser.add_argument("--frontend_thresh", type=float, default=12)
    parser.add_argument("--frontend_window", type=int, default=25)
    # 2
    parser.add_argument("--frontend_radius", type=int, default=2)
    parser.add_argument("--frontend_nms", type=int, default=1)

    parser.add_argument("--backend_thresh", type=float, default=15)
    parser.add_argument("--backend_radius", type=int, default=2)
    parser.add_argument("--backend_nms", type=int, default=3)

    parser.add_argument("--segm_filter", type=bool, default=False,
                        help="if filter weight with segmentaion in factory graph")
...
if __name__ == '__main__':
    ...
    for (t, image, intrinsics, segm, id_image) in tqdm(image_stream(args.datapath, mode='val', args=args)):
        droid.track(t, image, intrinsics=intrinsics, segments=segm, image_id=id_image)
    ...
    gt_file = os.path.join(args.datapath, 'clone/extrinsic.txt')
    traj_ref = read_vkitti2_poses_file(gt_file, args)

    pose = torch.tensor(traj_est)
    path = 'shared_data/traj/'
    path = os.path.join(path, args.datapath.rsplit('/')[-1])
    path = os.path.join(path, 'clone')
    if not os.path.isdir(path):
        os.makedirs(path)
    path = os.path.join(path, 'pose.txt')
    with open(path, 'w') as file:
        for matrix in pose:
            Ps = SE3(matrix).matrix().cpu().numpy()
            Ps = Ps.flatten()
            np.savetxt(file, Ps, delimiter=' ', newline=' ')
            file.write('\n')
